				...
				<div class="w-75 mx-auto">
					<div class="d-flex justify-center flex-column">
                        <div v-if="chatTillNow.length">
                            <render-chat v-for="chat in chatTillNow" :key="chat.key" :is-user-prompt="chat.isUserPrompt" :string-to-render="chat.stringToRender"/>
                        </div>
                        <div v-else>
                            ...
                            <div class="w-100 mt-5">
                                <div class="w-50 rounded-xl pa-5" style="border: 1px dashed green;">
                                    Sample Prompts
                                    <div class="w-100 d-flex flex-column" v-for="prompt in SAMPLE_PROMPTS" :key="prompt">
                                        <v-btn class="my-1" variant="outlined" @click="onClickExistingPrompt(prompt)"> {{ prompt }} </v-btn>
                                    </div>
                                </div>
                            </div>
                        </div>
